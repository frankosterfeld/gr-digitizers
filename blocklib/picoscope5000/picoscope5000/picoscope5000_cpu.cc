#include "picoscope5000_cpu.h"
#include "picoscope5000_cpu_gen.h"
...
struct PicoStatus5000Errc : std::error_category {
    const char* name() const noexcept override;
    std::string message(int ev) const override;
};

const char* PicoStatus5000Errc::name() const noexcept { return "Ps5000"; }

std::string PicoStatus5000Errc::message(int ev) const
{
    PICO_STATUS status = static_cast<PICO_STATUS>(ev);
    return detail::get_error_message(status);
}

const PicoStatus5000Errc thePsErrCategory{};
 ...
 */
std::error_code make_pico_5000_error_code(PICO_STATUS e)
{
...
namespace gr::picoscope5000 {

/*!
 * a structure used for streaming setup
 */
struct ps5000_unit_interval_t {
    PS5000_TIME_UNITS unit;
    uint32_t interval;
};
...
static PS5000_COUPLING convert_to_ps5000_coupling(coupling_t coupling)
{
...
static PS5000_RANGE convert_to_ps5000_range(double range)
{
    if (range == 0.01)
        return PS5000_10MV;
    else if (range == 0.02)
 ...
 */
uint32_t picoscope_5000_impl::convert_frequency_to_ps5000_timebase(double desired_freq,
                                                                     double& actual_freq)
{
    ...
    for (auto i = 0; i < 2; i++) {
        auto status = ps5000GetTimebase2(
...
ps5000_unit_interval_t
convert_frequency_to_ps5000_time_units_and_interval(double desired_freq,
                                                     double& actual_freq)
{
    ps5000_unit_interval_t unint;
    auto interval = 1.0 / desired_freq;

    if (interval < 0.000001) {
        unint.unit = PS5000_PS;
...
static PS5000_RATIO_MODE convert_to_ps5000_ratio_mode(downsampling_mode_t mode)
{
    switch (mode) {
    case downsampling_mode_t::MIN_MAX_AGG:
        return PS5000_RATIO_MODE_AGGREGATE;
    case downsampling_mode_t::DECIMATE:
        return PS5000_RATIO_MODE_DECIMATE;
    case downsampling_mode_t::AVERAGE:
        return PS5000_RATIO_MODE_AVERAGE;
    case downsampling_mode_t::NONE:
    default:
        return PS5000_RATIO_MODE_NONE;
    }
}

PS5000_THRESHOLD_DIRECTION
convert_to_ps5000_threshold_direction(trigger_direction_t direction)
{
    switch (direction) {
    case trigger_direction_t::RISING:
        return PS5000_RISING;
    case trigger_direction_t::FALLING:
        return PS5000_FALLING;
...
int16_t convert_voltage_to_ps5000_raw_logic_value(double value)
{
    ...
    return (int16_t)((value / max_logical_voltage) * (double)PS5000_EXT_MAX_VALUE);
}

PS5000_CHANNEL
convert_to_ps5000_channel(const std::string& source)
{
    if (source == "A") {
        return PS5000_CHANNEL_A;
    }
    else if (source == "B") {
        return PS5000_CHANNEL_B;
    }
    else if (source == "C") {
        return PS5000_CHANNEL_C;
    }
    else if (source == "D") {
        return PS5000_CHANNEL_D;
    }
    else if (source == "EXTERNAL") {
        return PS5000_EXTERNAL;
    }
    else {
        // return invalid value
        return PS5000_MAX_TRIGGER_SOURCES;
    }
}
...
picoscope_5000_impl::picoscope_5000_impl(const digitizers::digitizer_args& args,
...
picoscope_5000_impl::~picoscope_5000_impl() { driver_close(); }

/**********************************************************************
 * Driver implementation
 *********************************************************************/

std::string picoscope_5000_impl::get_unit_info_topic(PICO_INFO info) const
{
    char line[40];
    int16_t required_size;

    auto status = ps5000GetUnitInfo(
...
std::string picoscope_5000_impl::get_driver_version() const
{
    const std::string prefix = "PS5000 Linux Driver, ";
...
std::string picoscope_5000_impl::get_hardware_version() const
{
    if (!d_initialized)
        return "NA";
...
static constexpr PS5000_DEVICE_RESOLUTION map_resolution(digitizer_resolution_t r)
{
    switch (r) {
    case digitizer_resolution_t::R_8BIT:
        return PS5000_DR_8BIT;
    case digitizer_resolution_t::R_12BIT:
        return PS5000_DR_12BIT;
    case digitizer_resolution_t::R_14BIT:
        return PS5000_DR_14BIT;
    case digitizer_resolution_t::R_15BIT:
        return PS5000_DR_15BIT;
    case digitizer_resolution_t::R_16BIT:
        return PS5000_DR_16BIT;
    }

    return PS5000_DR_8BIT;
}

std::error_code picoscope_5000_impl::driver_initialize()
{
    PICO_STATUS status;
    ...
    if (d_serial_number.empty()) {
        status = ps5000OpenUnit(&(d_handle), NULL, resolution);
    }
    else {
        status = ps5000OpenUnit(&(d_handle), (int8_t*)d_serial_number.c_str(), resolution);
    }
    ...
    if (status == PICO_POWER_SUPPLY_NOT_CONNECTED ||
        ...
        status = ps5000ChangePowerSource(d_handle, status);
        if (status == PICO_POWER_SUPPLY_NOT_CONNECTED ||
            status == PICO_USB3_0_DEVICE_NON_USB3_0_PORT) {
            status = ps5000ChangePowerSource(d_handle, status);
        }
    ...
    if (status != PICO_OK) {
        ...
        return make_pico_5000_error_code(status);
    }

    // maximum value is used for conversion to volts
    status = ps5000MaximumValue(d_handle, &d_max_value);
    if (status != PICO_OK) {
        ps5000CloseUnit(d_handle);
        d_logger->error("ps5000MaximumValue: {}", detail::get_error_message(status));
        return make_pico_5000_error_code(status);
    }
...
std::error_code picoscope_5000_impl::driver_configure()
{
    assert(d_ai_channels <= PS5000_MAX_CHANNELS);

    int32_t max_samples;
    PICO_STATUS status = ps5000MemorySegments(d_handle, d_nr_captures, &max_samples);
    if (status != PICO_OK) {
        d_logger->error("ps5000MemorySegments: {}", detail::get_error_message(status));
        return make_pico_5000_error_code(status);
    }

    if (d_acquisition_mode == acquisition_mode_t::RAPID_BLOCK) {
        status = ps5000SetNoOfCaptures(d_handle, d_nr_captures);
        if (status != PICO_OK) {
            d_logger->error("ps5000SetNoOfCaptures: {}",
                            detail::get_error_message(status));
            return make_pico_5000_error_code(status);
        }
    }
    ...
    for (auto i = 0; i < d_ai_channels; i++) {
        ...
        auto coupling = convert_to_ps5000_coupling(d_channel_settings[i].coupling);
        auto range = convert_to_ps5000_range(d_channel_settings[i].range);
        auto offset = d_channel_settings[i].offset;

        status = ps5000SetChannel(d_handle,
                                   static_cast<PS5000_CHANNEL>(i),
                                   enabled,
                                   coupling,
                                   range,
                                   offset);
        if (status != PICO_OK) {
            d_logger->error(
                "ps5000SetChannel (chan {}): {}", i, detail::get_error_message(status));
            return make_pico_5000_error_code(status);
        }
    }
    ...
    if (d_trigger_settings.is_analog() &&
        ...
        status = ps5000SetSimpleTrigger(
            d_handle,
            true, // enable
            convert_to_ps5000_channel(d_trigger_settings.source),
            convert_voltage_to_ps5000_raw_logic_value(d_trigger_settings.threshold),
            convert_to_ps5000_threshold_direction(d_trigger_settings.direction),
            0,   // delay
            -1); // auto trigger
        if (status != PICO_OK) {
            d_logger->error("ps5000SetSimpleTrigger: {}",
                            detail::get_error_message(status));
            return make_pico_5000_error_code(status);
        }
    }
        ...
        // disable triggers...
        for (int i = 0; i < PS5000_MAX_CHANNELS; i++) {
            PS5000_CONDITION cond;
            cond.source = static_cast<PS5000_CHANNEL>(i);
            cond.condition = PS5000_CONDITION_DONT_CARE;
            status =
                ps5000SetTriggerChannelConditionsV2(d_handle, &cond, 1, PS5000_CLEAR);
            if (status != PICO_OK) {
                d_logger->error("ps5000SetTriggerChannelConditionsV2: {}",
                                detail::get_error_message(status));
                return make_pico_5000_error_code(status);
            }
        }
    ...
    double actual_freq;
    convert_frequency_to_ps5000_timebase(d_samp_rate, actual_freq);

    return std::error_code{};
}

void rapid_block_callback_redirector_5000(int16_t handle, PICO_STATUS status, void* vobj)
{
    static_cast<picoscope_5000_impl*>(vobj)->rapid_block_callback(handle, status);
}

void picoscope_5000_impl::rapid_block_callback(int16_t handle, PICO_STATUS status)
{
    auto errc = make_pico_5000_error_code(status);
    notify_data_ready(errc);
}

std::error_code picoscope_5000_impl::driver_arm()
{
    if (d_acquisition_mode == acquisition_mode_t::RAPID_BLOCK) {
        uint32_t timebase =
            convert_frequency_to_ps5000_timebase(d_samp_rate, d_actual_samp_rate);

        auto status =
            ps5000RunBlock(d_handle,
                            d_pre_samples,  // pre-triggersamples
                            d_post_samples, // post-trigger samples
                            timebase,       // timebase
                            NULL,           // time indispossed
                            0,              // segment index
                            (ps5000BlockReady)rapid_block_callback_redirector_5000,
                            this);
        if (status != PICO_OK) {
            d_logger->error("ps5000RunBlock: {}", detail::get_error_message(status));
            return make_pico_5000_error_code(status);
        }
    }
    else {
        ...
        ps5000_unit_interval_t unit_int =
            convert_frequency_to_ps5000_time_units_and_interval(d_samp_rate,
                                                                 d_actual_samp_rate);

        auto status =
            ps5000RunStreaming(d_handle,
                                ...
                                convert_to_ps5000_ratio_mode(d_downsampling_mode),
                                d_driver_buffer_size);

        if (status != PICO_OK) {
            d_logger->error("ps5000RunStreaming: {}", detail::get_error_message(status));
            return make_pico_5000_error_code(status);
        }
    }
...
std::error_code picoscope_5000_impl::driver_disarm()
{
    auto status = ps5000Stop(d_handle);
    if (status != PICO_OK) {
        d_logger->error("ps5000Stop: {}", detail::get_error_message(status));
    }

    return make_pico_5000_error_code(status);
}

std::error_code picoscope_5000_impl::driver_close()
{
    if (d_handle == -1) {
        return std::error_code{};
    }

    auto status = ps5000CloseUnit(d_handle);
    if (status != PICO_OK) {
        d_logger->error("ps5000CloseUnit: {}", detail::get_error_message(status));
    }
...
std::error_code picoscope_5000_impl::set_buffers(size_t samples, uint32_t block_number)
{
    PICO_STATUS status;
    ...
    for (auto aichan = 0; aichan < d_ai_channels; aichan++) {
        ...
        if (d_downsampling_mode == downsampling_mode_t::MIN_MAX_AGG) {
            ...
            status =
                ps5000SetDataBuffers(d_handle,
                                      static_cast<PS5000_CHANNEL>(aichan),
                                      &d_buffers[aichan][0],
        ...
        if (status != PICO_OK) {
            d_logger->error("ps5000SetDataBuffer (chan {}): {}",
                            aichan,
                            detail::get_error_message(status));
            return make_pico_5000_error_code(status);
        }
...
std::error_code picoscope_5000_impl::driver_prefetch_block(size_t samples,
                                                            size_t block_number)
{
    ...
    auto status = ps5000GetValues(d_handle,
                                   0, // offset
                                   &nr_samples,
                                   d_downsampling_factor,
                                   convert_to_ps5000_ratio_mode(d_downsampling_mode),
                                   ...
                                   &d_overflow);
    if (status != PICO_OK) {
        d_logger->error("ps5000GetValues: {}", detail::get_error_message(status));
    }
...
std::error_code
picoscope_5000_impl::driver_get_rapid_block_data(size_t offset,
                                                  size_t length,
                                                  size_t waveform,
                                                  work_io& wio,
...
std::error_code picoscope_5000_impl::driver_poll()
{
    auto status = ps5000GetStreamingLatestValues(
        d_handle,
        (ps5000StreamingReady)digitizers::invoke_streaming_callback,
...
picoscope5000_cpu::picoscope5000_cpu(block_args args)
    : INHERITED_CONSTRUCTORS,
      d_impl(
          { .sample_rate = args.sample_rate,
            ...
            .ai_channels = PS5000_MAX_CHANNELS,
            .ports = 0 },
          args.resolution,
          args.serial_number,
          d_logger)
...
bool picoscope5000_cpu::start() { return d_impl.start(); }

bool picoscope5000_cpu::stop() { return d_impl.stop(); }

work_return_t picoscope5000_cpu::work(work_io& wio) { return d_impl.work(wio); }

void picoscope5000_cpu::initialize() { d_impl.initialize(); }

void picoscope5000_cpu::close() { d_impl.close(); }
